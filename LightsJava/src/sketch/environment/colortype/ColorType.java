...
public interface ColorType {
  ...
  public static EditSlider[] addColorSliders(EditPanel editPanel, Color initialColor, Consumer<Float> redConsumer,
    ...
    sliders[0] = editPanel.addInput(new EditSlider(0, 255, initialColor.getRed(), editPanel).setControlling(redConsumer)
        ...
        .setHandleSize(20, 30).styleTrack(new DynamicPaint(redSliderPaint), new WeightedStroke(Color.BLACK, 2)));
    sliders[1] = editPanel
        .addInput(new EditSlider(0, 255, initialColor.getGreen(), editPanel).setControlling(greenConsumer)
            .setPosition(10, editPanel.getNextAvailableY() + 10).setSize(editPanel.getWidth() - 20, 20)
            .setHandleSize(20, 30).styleTrack(new DynamicPaint(greenSliderPaint), new WeightedStroke(Color.BLACK, 2)));
    sliders[2] = editPanel
        .addInput(new EditSlider(0, 255, initialColor.getBlue(), editPanel).setControlling(blueConsumer)
            .setPosition(10, editPanel.getNextAvailableY() + 10).setSize(editPanel.getWidth() - 20, 20)
            .setHandleSize(20, 30).styleTrack(new DynamicPaint(blueSliderPaint), new WeightedStroke(Color.BLACK, 2)));
    return sliders;
  }
}
